...
def copy_data(args):
    ...
    try:
        with nc.Dataset(in_f) as in_fp, nc.Dataset(out_f, "r+") as out_fp:
            ...
            out_fp.setncatts(
                get_provenance_metadata(output_filename=out_f, write_readme_file=False)
            )
            out_fp.setncattr(
                "template_file",
                f"{os.path.abspath(template_f)} (md5 hash: {md5sum(template_f)})",
            )
            out_fp.setncattr(
                "old_restart_file",
                f"{os.path.abspath(in_f)} (md5 hash: {md5sum(in_f)})",
            )

            for var in in_fp.variables:
                shape = in_fp.variables[var].shape
                print(f"  [CHECK] var: {var}, shape: {shape}")
                if len(shape) != 4 or shape[0] != 1 or shape[1] != NLEVELS:
                    print(f"  [SKIP] {var}: not 4D with {NLEVELS} levels")
                    continue
...
def main():
    ...
    # Write a single README describing the whole restart folder
    get_provenance_metadata(
        input_files=old_files + template_files,
