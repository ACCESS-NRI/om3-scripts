...
def unmask_file(filename, mask=None, missing_value=None, skip_vars=[]):
    with nc.Dataset(filename, 'r+') as f:
        ...
        # Add metadata
        f.setncattr("title", "Coupler restart fields updated with land mask of modified bathymetry")
...
def main():
    ...
    parser.add_argument("--mask_var", default="kmt",
                        help="Name of the mask variable in the mask file (default: kmt)")
    args = parser.parse_args()

    skip_vars = ['time', 'time_bnds', 'start_ymd', 'start_tod',
                 'curr_ymd', 'curr_tod', 'ocnExpAccum_cnt']
    missing_value = 1e30
