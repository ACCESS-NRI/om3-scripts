...
def copy_data(args):
    ...
    try:
        with nc.Dataset(in_f) as in_fp, nc.Dataset(out_f, 'r+') as out_fp:
            ...
            out_fp.setncattr("history", f"Updated on {datetime.now().strftime('%Y-%m-%d')}")
...
def main():
    ...
    parser.add_argument("template_dir", help="Name of directory containing collated restart files from a run using the new bathymetry (typically a short run from rest).")
    parser.add_argument("old_dir", help="Name of directory containing collated restart files from a run using the old bathymetry.")
    parser.add_argument("output_dir", help="Name of the output directory which will contain new restarts.")
    parser.add_argument("template_prefix", help="File name prefix of template restart files from the run using the new bathymetry "
    ...
    parser.add_argument("old_prefix", help="File name prefix of old restart files from the run using the old bathymetry. "
    ...
    template_files = sorted(glob(os.path.join(args.template_dir, f"{args.template_prefix}*.nc")))
