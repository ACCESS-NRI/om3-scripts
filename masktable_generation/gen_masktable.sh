...
# Identify which commit of this script generated the table, for provenance.
SCRIPT_PATH=$(readlink -f "${BASH_SOURCE[0]}")
SCRIPT_DIR=$(dirname "${SCRIPT_PATH}")
SCRIPT_COMMIT="unknown (not run from a git checkout)"
if git -C "${SCRIPT_DIR}" rev-parse --is-inside-work-tree &>/dev/null; then
    if SCRIPT_COMMIT=$(
        git -C "${SCRIPT_DIR}" log -1 --format='%h (%ad) "%s"' --date=short -- "${SCRIPT_PATH}" 2>/dev/null
    ) && [[ -n "${SCRIPT_COMMIT}" ]]; then
        if ! git -C "${SCRIPT_DIR}" diff --quiet -- "${SCRIPT_PATH}" 2>/dev/null; then
            SCRIPT_COMMIT="${SCRIPT_COMMIT} [locally modified]"
        fi
    else
        SCRIPT_COMMIT="unknown (script not tracked in this checkout)"
    fi
fi
...
PROVENANCE_COMMON=(
    ...
    "Date: $(TZ='Australia/Melbourne' date +'%Y-%m-%dT%H:%M:%S%z')"
