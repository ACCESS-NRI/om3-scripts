...
def build_yearly_file(source_files, stream, year, out_path, varname, history_str):
    ...
    with netCDF4.Dataset(source_files[0]) as s0:
        ...
        lon_attrs     = {a: getattr(s0.variables["longitude"], a)
                         for a in s0.variables["longitude"].ncattrs()}
        lat_attrs     = {a: getattr(s0.variables["latitude"],  a)
                         for a in s0.variables["latitude"].ncattrs()}
        var_attrs     = {a: getattr(s0.variables[varname], a)
    ...
    with netCDF4.Dataset(out_path, "w", format="NETCDF4") as dst:
        ...
        v_data = dst.createVariable(
            varname, "i2", ("time", "latitude", "longitude"),
            zlib=True, shuffle=True, complevel=COMPLEVEL,
...
def validate(out_path, source_files, varname):
    ...
    with netCDF4.Dataset(out_path) as dst:
        ...
        if chunks != [CHUNK_T, CHUNK_LAT, CHUNK_LON]:
            errors.append(f"chunking wrong: got {chunks}, expected [{CHUNK_T},{CHUNK_LAT},{CHUNK_LON}]")
        ...
        for attr in ("rechunked_from", "rechunked_by", "rechunked_date",
                     "original_chunking", "target_chunking"):
...
def process_one(stream, year, source_base, output_base, overwrite, runcmd):
    ...
    try:
        t0      = time.time()
        n_t     = build_yearly_file(source_files, stream, year, tmp_path, varname, history_str)
...
def _output_exists(stream, year, output_base):
    ...
    import glob
    return bool(
        glob.glob(os.path.join(output_base, stream, f"{stream}_era5_oper_sfc_{year}*-*.nc"))
    )
...
def main():
    ...
    if args.stream and args.year:
        ...
        ok = process_one(
            args.stream, args.year,
            args.source_dir, args.output_dir,
            args.overwrite, runcmd,
        )
    ...
    for stream in streams:
        for year in years:
            if _output_exists(stream, year, args.output_dir) and not args.overwrite:
                ...
                tasks.append((stream, year, args.source_dir, args.output_dir, args.overwrite))
